...
static void GetImage(std::string_view file, int& width, int& height, std::vector<uint32_t>& image)
{
	int nrChannels;

	uint32_t* data = (uint32_t*)stbi_load(file.data(), &width, &height, &nrChannels, STBI_rgb_alpha);
	if (!data)
		return;
